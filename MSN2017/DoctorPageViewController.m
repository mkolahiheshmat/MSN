...
@interface DoctorPageViewController()<UITableViewDelegate, UITableViewDataSource, UIScrollViewDelegate, MFMailComposeViewControllerDelegate, UIGestureRecognizerDelegate, AVAudioPlayerDelegate>
{
...
- (void)makeTopBar{
    ...
    titleLabel.text = NSLocalizedString(@"doctorPage", @"");
...
- (void)makeSimilarDoctorView{
    ...
    similarLabelOnBottom.text = NSLocalizedString(@"similarDoctor", @"");
...
- (void)makeTableViewWithYpos:(CGFloat )yPos{
    ...
    [aboutDocButton setTitle:NSLocalizedString(@"aboutDoc", @"") forState:UIControlStateNormal];
...
- (void)tapOnSilimilarDoctorImage:(UITapGestureRecognizer *)tap{
    ...
    UIStoryboard *story = [UIStoryboard storyboardWithName:@"Main" bundle:nil];
    DoctorPageViewController *view = (DoctorPageViewController *)[story instantiateViewControllerWithIdentifier:@"DoctorPageViewController"];
    view.userEntityId = [[similarDoctorsArray objectAtIndex:tap.view.tag]objectForKey:@"doctor_id"];
    view.userAvatar = [[similarDoctorsArray objectAtIndex:tap.view.tag]objectForKey:@"photo"];
    view.userTitle = [[similarDoctorsArray objectAtIndex:tap.view.tag]objectForKey:@"full_name"];
    view.userJobTitle = [[similarDoctorsArray objectAtIndex:tap.view.tag]objectForKey:@"expertise"];
    [self.navigationController pushViewController:view animated:YES];
...
- (void)fetchDoctorInfoWithEntityId:(NSString *)entityId{
    NSDictionary *params = @{@"doctor_id":entityId};
    /*http://213.233.175.250:8081/web_services/v3/page/aboutDoctor*/
    
    NSString *url = [NSString stringWithFormat:@"%@page/aboutDoctor", BaseURL];
    ...
    if (!isBusyNow) {
        ...
        [manager POST:url parameters:params progress:nil success:^(NSURLSessionTask *operation, id responseObject) {
            NSDictionary *tempDic = (NSDictionary *)responseObject;
            NSMutableDictionary *tempDic2 = [[NSMutableDictionary alloc]initWithDictionary:tempDic];
            ...
            if (_isFromPushNotif) {
                [doctorImageView setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@", [tempDic objectForKey:@"avatar"]]] placeholderImage:[UIImage imageNamed:@"icon upload ax"]];
            
                authorNameLabel.text = [tempDic objectForKey:@"name"];
                authorJobLabel.text = [tempDic objectForKey:@"job_title"];
...
- (void)fetchSimilarDoctorsWithDoctorId:(NSString *)doctorId{
    NSDictionary *params = @{@"doctor_id":doctorId,
                             @"limit":@"",
                             @"offset":@""};
    /*http://213.233.175.250:8081/web_services/v3/expertises/sameExpertise*/
    
    NSString *url = [NSString stringWithFormat:@"%@expertises/sameExpertise", BaseURL];
    AFHTTPSessionManager *manager = [AFHTTPSessionManager manager];
    manager.responseSerializer.acceptableContentTypes = [manager.responseSerializer.acceptableContentTypes setByAddingObject:@"text/html"];
    manager.requestSerializer.timeoutInterval = 45;;
    [manager POST:url parameters:params progress:nil success:^(NSURLSessionTask *operation, id responseObject) {
        ...
        CGFloat xPos = 0;
        for (NSDictionary *doctorDic in tempDic) {
            ...
            [docImageView setImageWithURL:[NSURL URLWithString:[doctorDic objectForKey:@"photo"]] placeholderImage:[UIImage imageNamed:@"icon upload ax"]];
            ...
            nameLabel.text = [doctorDic objectForKey:@"full_name"];
            ...
            jobLabel.text = [doctorDic objectForKey:@"expertise"];
