...
@interface UserProfileViewController ()<UITableViewDelegate, UITableViewDataSource, UIGestureRecognizerDelegate, AVAudioPlayerDelegate, UIWebViewDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate>
{
    BOOL    isBusyNow;
...
- (void)makeTopBar{
    ...
    titleLabel =[[UILabel alloc]initWithFrame:CGRectMake(40, topViewHeight/2 - 15, screenWidth - 125, 40)];
...
- (void)makeImageAndButtons{
    ...
    followeeButton = [CustomButton initButtonWithTitle:@"0" withTitleColor:[UIColor blueColor] withBackColor:[UIColor whiteColor] withFrame:CGRectMake(2*screenWidth/3, usernameLabel.frame.origin.y + 50, 110, 30)];
    ...
    followersButton = [CustomButton initButtonWithTitle:@"0" withTitleColor:[UIColor blueColor] withBackColor:[UIColor whiteColor] withFrame:CGRectMake(screenWidth/3, usernameLabel.frame.origin.y + 50, 110, 30)];
    ...
    UITapGestureRecognizer *tap2 = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(followersButtonAction)];
    ...
    UITapGestureRecognizer *tap3 = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(followeeButtonAction)];
...
- (void)editProfileButtonAction{
    if (canEdit) {
        ...
        [self followUserConnection:_entityID];
    }
}
...
- (void)followimageViewActionForCompany:(UITapGestureRecognizer *)tap{
    ...
    NSDictionary *dic = [companyTableArray objectAtIndex:tap.view.tag];
...
- (void)followimageViewActionForProject:(UITapGestureRecognizer *)tap{
    ...
    //NSLog(@"%@", [dic objectForKey:@"id"]);
    [self followProjectConnection:[[dic objectForKey:@"id"]integerValue] row:tap.view.tag];
}
...
- (void)followersButtonAction{
    ...
    view.isFollowee = NO;
    view.isFromComORProj = NO;
    view.entityID = [[_dictionary objectForKey:@"entity_id"]integerValue];
    if (view.entityID == 0) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (tableView.tag == 971){
        ...
        if (indexPath.row < [companyTableArray count]) {
            NSDictionary *searchDic = [companyTableArray objectAtIndex:indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (tableView.tag == 971) {
        ...
        NSDictionary *dic = [companyTableArray objectAtIndex:indexPath.row];
        ...
        NSDictionary *dic = [projectTableArray objectAtIndex:indexPath.row];
...
{
    [ProgressHUD show:@""];
    NSDictionary *params = @{@"entity_id":[NSNumber numberWithInteger:_entityID]
                             };
    ...
    [manager POST:url parameters:params constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
        ...
        if ([[resDic objectForKey:@"success"]integerValue] == 1) {
            ...
            for (NSDictionary *dic in [resDic objectForKey:@"data"]) {
                ...
                } else if ([[dic objectForKey:@"entity_type_id"]integerValue] == 3)/*company profile = 2*/ {
                    [companyTableArray addObject:dic];
                } else if ([[dic objectForKey:@"entity_type_id"]integerValue] == 2)/*project profile = 3*/ {
            ...
            if ([responseArray count] > 0) {
                /*
                 ...
                 birthdateTextField.text = [[responseArray lastObject]objectForKey:@"birth_date"];
                 emailTextField.text = [[responseArray lastObject]objectForKey:@"email"];
                 mobileTextField.text = [[responseArray lastObject]objectForKey:@"mobile"];
                 educationTextField.text = [[responseArray lastObject]objectForKey:@"education"];
                 savabeghTextField.text = [[responseArray lastObject]objectForKey:@"work_experience"];
                 */
            }
        }
    } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
...
-(void)getProfileDetailFromServer
{
    ...
    [manager POST:url parameters:params constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
        ...
        if ([[resDic objectForKey:@"success"]integerValue] == 1) {
            ...
            [followeeButton setTitle:[NSString stringWithFormat:@"%ld", (long)followersCount] forState:UIControlStateNormal];
            
            NSInteger followeeCount = [[[resDic objectForKey:@"data"]objectForKey:@"following_count"]integerValue];
            [followersButton setTitle:[NSString stringWithFormat:@"%ld", (long)followeeCount] forState:UIControlStateNormal];
            ...
            [profileImageView setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@", [[resDic objectForKey:@"data"] objectForKey:@"avatar"]]]];
            nameTextField.text = [[resDic objectForKey:@"data"] objectForKey:@"name"];
            birthdateTextField.text = [[resDic objectForKey:@"data"] objectForKey:@"birth_date"];
            emailTextField.text = [[resDic objectForKey:@"data"] objectForKey:@"email"];
            mobileTextField.text = [[resDic objectForKey:@"data"] objectForKey:@"mobile"];
            //educationTextField.text = [[resDic objectForKey:@"data"] objectForKey:@"education"];
            //savabeghTextField.text = [[resDic objectForKey:@"data"] objectForKey:@"work_experience"];
            //====================================================================
            NSString *educationStr = [[resDic objectForKey:@"data"] objectForKey:@"education"];
            educationLabel =[[UILabel alloc]initWithFrame:CGRectMake(20, -20, screenWidth - 40, [self getHeightOfString:educationStr] + 20)];
            educationLabel.font = FONT_NORMAL(16);
            educationLabel.text = educationStr;
            ...
            //educationLabel.backgroundColor = [UIColor cyanColor];
            [savabeghView addSubview:educationLabel];
            //63 × 30
            ...
            UIView *horizontalLineForEducationLabel = [[UIView alloc]initWithFrame:CGRectMake(20, educationLabel.frame.origin.y + educationLabel.frame.size.height - 20, screenWidth - 40, 0.5)];
            horizontalLineForEducationLabel.backgroundColor = [UIColor lightGrayColor];
            [savabeghView addSubview:horizontalLineForEducationLabel];
            
            NSString *savabeghStr = [[resDic objectForKey:@"data"] objectForKey:@"work_experience"];
            savabeghLabel2 =[[UILabel alloc]initWithFrame:CGRectMake(20, horizontalLineForEducationLabel.frame.origin.y + horizontalLineForEducationLabel.frame.size.height, screenWidth - 40, [self getHeightOfString:savabeghStr] + 20)];
            savabeghLabel2.font = FONT_NORMAL(16);
            savabeghLabel2.text = savabeghStr;
            savabeghLabel2.numberOfLines = 0;
            savabeghLabel2.textColor = [UIColor blackColor];
            savabeghLabel2.textAlignment = NSTextAlignmentCenter;
            //savabeghLabel2.backgroundColor = [UIColor redColor];
            [savabeghView addSubview:savabeghLabel2];
            //56x46
            UIImageView *savabeghImageView = [[UIImageView alloc]initWithFrame:CGRectMake(screenWidth - 40, horizontalLineForEducationLabel.frame.origin.y + horizontalLineForEducationLabel.frame.size.height + 30, 56 / 3, 46 / 3)];
            ...
            UIView *horizontalLineForSavabeghLabel = [[UIView alloc]initWithFrame:CGRectMake(20, savabeghLabel2.frame.origin.y + savabeghLabel2.frame.size.height - 20, screenWidth - 40, 0.5)];
            horizontalLineForSavabeghLabel.backgroundColor = [UIColor lightGrayColor];
            [savabeghView addSubview:horizontalLineForSavabeghLabel];
            if ([savabeghStr length] == 0 && [educationStr length]== 0) {
                scrollView.contentSize = CGSizeMake(screenWidth, savabeghView.frame.origin.y + savabeghView.frame.size.height);
            }else{
                scrollView.contentSize = CGSizeMake(screenWidth, savabeghView.frame.origin.y + savabeghView.frame.size.height + 350);
            }
            
            //=====================================================================
            
            jobPositionTextField.text = [[[resDic objectForKey:@"data"]objectForKey:@"job_title"]objectForKey:@"name"];
            selectedJobTileID = [[[[resDic objectForKey:@"data"]objectForKey:@"job_title"]objectForKey:@"id"]integerValue];
...
-(void)followUserConnection:(NSInteger)profileId
{
    [ProgressHUD show:@""];
...
-(void)followCompanyConnection:(NSInteger)profileId row:(NSInteger)row
{
    ...
    [manager POST:url parameters:params constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
        ...
        if ([[resDic objectForKey:@"success"]integerValue] == 1) {
            ...
            ProjectCompanyCustomCell *cell = (ProjectCompanyCustomCell *)[companyTableView cellForRowAtIndexPath:indexpath];//[companyTableArray objectAtIndex:row];
...
-(void)followProjectConnection:(NSInteger)profileId row:(NSInteger)row
{
    ...
    [manager POST:url parameters:params constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
        ...
        if ([[resDic objectForKey:@"success"]integerValue] == 1) {
            //reload related cell
            NSIndexPath *indexpath = [NSIndexPath indexPathForRow:row inSection:0];
            ProjectCompanyCustomCell *cell = (ProjectCompanyCustomCell *)[projectTableView cellForRowAtIndexPath:indexpath];//[companyTableArray objectAtIndex:row];
            cell.followimageView.image = [UIImage imageNamed:@"IWillUnfollow"];
        }
        UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"" message:[resDic objectForKey:@"message"] preferredStyle:UIAlertControllerStyleAlert];
        UIAlertAction *defaultAction = [UIAlertAction actionWithTitle:@"بازگشت" style:UIAlertActionStyleDefault handler:^(UIAlertAction * action) {
