...
@interface ClinicViewController()<UITableViewDelegate, UITableViewDataSource, UIScrollViewDelegate, MFMailComposeViewControllerDelegate, UIGestureRecognizerDelegate, AVAudioPlayerDelegate>
{
...
- (void)viewDidLoad {
    ...
    [super viewDidLoad];
    [self fetchClinicInfoWithEntityId:self.userEntityId];
    [self fetchPostsFromServerWithRequest:@"" WithDate:@""];
    [self fetchDoctorOfClinicWithClinicId:self.userEntityId];
...
- (void)makeTopBar{
    ...
    titleLabel.text = NSLocalizedString(@"clinicPage", @"");
...
- (void)makeSimilarDoctorView{
    ...
    similarLabelOnBottom.text = NSLocalizedString(@"similarClinic", @"");
...
- (void)makeTableViewWithYpos:(CGFloat )yPos{
    ...
    [aboutDocButton setTitle:NSLocalizedString(@"aboutClinic", @"") forState:UIControlStateNormal];
...
- (void)fetchClinicInfoWithEntityId:(NSString *)entityId{
    NSDictionary *userpassDic = [GetUsernamePassword getUsernamePassword];
...
- (void)fetchDoctorOfClinicWithClinicId:(NSString *)clinicId{
    NSDictionary *userpassDic = [GetUsernamePassword getUsernamePassword];
    NSDictionary *params = @{@"username":[userpassDic objectForKey:@"username"]/*@"09191234072"*/,
                             @"password":[userpassDic objectForKey:@"password"]/*@“123456”*/,
                             @"unit_id":@"3",
    ...
    [manager POST:url parameters:params progress:nil success:^(NSURLSessionTask *operation, id responseObject) {
        ...
        CGFloat xPos = 0;
        for (NSDictionary *doctorDic in [tempDic objectForKey:@"doctors"]) {
