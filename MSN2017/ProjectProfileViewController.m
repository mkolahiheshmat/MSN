...
@interface ProjectProfileViewController ()<UITableViewDelegate, UITableViewDataSource, UIGestureRecognizerDelegate, AVAudioPlayerDelegate, UIWebViewDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate>
{
    BOOL    isBusyNow;
    ...
    UIImageView *showMembersButtonImageView;
    UIImageView *showCompanyButtonImageView;
    UIButton *postButton1;
    UIButton *infoButton;
    UIButton *showMembersButton;
...
- (void)makeTopBar{
    ...
    titleLabel.text = NSLocalizedString(@"projectProfile", @"");
...
- (void)makeImageAndButtons{
    ...
    UILabel *followersLabel =[[UILabel alloc]initWithFrame:CGRectMake(screenWidth/3 + 30, postButton.frame.origin.y + 20, 110, 40)];
...
- (void)makeTabBarButtons{
    ...
    //UIButton *postButton1 = [CustomButton initButtonWithImage:[UIImage imageNamed:@"tabbar"] withFrame:CGRectMake(0, usernameLabel.frame.origin.y + 110, screenWidth / 3, 30)];
    postButton1 = [CustomButton initButtonWithTitle:@"" withTitleColor:[UIColor clearColor] withBackColor:TABBAR isRounded:NO withFrame:CGRectMake(0, usernameLabel.frame.origin.y + 110, screenWidth / 3, 30)];
    ...
    //UIButton *infoButton = [CustomButton initButtonWithImage:[UIImage imageNamed:@"tabbar"] withFrame:CGRectMake(screenWidth / 3, usernameLabel.frame.origin.y + 110, screenWidth / 3, 30)];
    infoButton = [CustomButton initButtonWithTitle:@"" withTitleColor:[UIColor clearColor] withBackColor:TABBAR isRounded:NO withFrame:CGRectMake(screenWidth / 3, usernameLabel.frame.origin.y + 110, screenWidth / 3, 30)];
    ...
    //UIButton *showMembersButton = [CustomButton initButtonWithImage:[UIImage imageNamed:@"tabbar"] withFrame:CGRectMake(2 * screenWidth / 3 , usernameLabel.frame.origin.y + 110, screenWidth / 3, 30)];
    
    showMembersButton = [CustomButton initButtonWithTitle:@"" withTitleColor:[UIColor clearColor] withBackColor:TABBAR isRounded:NO withFrame:CGRectMake(2 * screenWidth / 3 , usernameLabel.frame.origin.y + 110, screenWidth / 3, 30)];
    [showMembersButton addTarget:self action:@selector(showMembersButtonAction) forControlEvents:UIControlEventTouchUpInside];
    [scrollView addSubview:showMembersButton];
    
    showMembersButtonImageView = [[UIImageView alloc]initWithFrame:CGRectMake(showMembersButton.frame.size.width/2 - (213/8), 0, 213 / 4, 100 / 4)];
    showMembersButtonImageView.image = [UIImage imageNamed:@"members_ios"];
    [showMembersButton addSubview:showMembersButtonImageView];
...
- (void)makeContactViewWithYpos:(CGFloat)yPos{
    ...
    //website
    [websiteTextField removeFromSuperview];
...
- (void)showPostButtonAction{
    ...
    showMembersButtonImageView.image = [UIImage imageNamed:@"members_ios"];
    [showMembersButton setBackgroundColor:TABBAR];
}
...
- (void)showMembersButtonAction{
    ...
    showMembersButtonImageView.image = [UIImage imageNamed:@"members_ios_selected"];
    [showMembersButton setBackgroundColor:TABBAR_SELECTED];
...
//    [view addSubview:aboutLabel];
    ...
    namesScrollViewForExpertise = [[UIScrollView alloc]initWithFrame:CGRectMake(0, takhasosLabel.frame.origin.y + 35, screenWidth, 40)];
...
- (void)deleteButtonAction{
    [self dismissTextView];
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"حذف پروفایل پروژه" message:@"آیا اطمینان دارید؟" preferredStyle:UIAlertControllerStyleAlert];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (tableView.tag == 971) {
        ...
        NSDictionary *dic = [membersTableArray objectAtIndex:indexPath.row];
        UIStoryboard *story = [UIStoryboard storyboardWithName:@"Main" bundle:nil];
        UserProfileViewController *view = (UserProfileViewController *)[story instantiateViewControllerWithIdentifier:@"UserProfileViewController"];
        view.entityID = [[dic objectForKey:@"id"]integerValue];
        view.dictionary = dic;
        [self.navigationController pushViewController:view animated:YES];
    }
    ...
    [manager POST:url parameters:nil progress:nil success:^(NSURLSessionTask *operation, id responseObject) {
        ...
        if ([[resDic objectForKey:@"success"]integerValue] == 1) {
            ...
            /*
             ...
             nameTextField.text = [[responseArray lastObject]objectForKey:@"full_name"];
             aboutTextField.text = [[responseArray lastObject]objectForKey:@"birth_date"];
...
- (void)getMembersFromServer{
    [ProgressHUD show:@""];
    NSDictionary *params = @{@"owner_id":[NSNumber numberWithInteger:[[_dictionary objectForKey:@"id"]integerValue]]
                             };
    NSString *url = [NSString stringWithFormat:@"%@api/entity/membership/members", BaseURL];
    AFHTTPSessionManager *manager = [AFHTTPSessionManager manager];
    manager.responseSerializer.acceptableContentTypes = [manager.responseSerializer.acceptableContentTypes setByAddingObject:@"application/json"];
    manager.requestSerializer.timeoutInterval = 45;
    //NSLog(@"%@", [GetUsernamePassword getAccessToken]);
    ...
    [manager POST:url parameters:params constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
        ...
        if ([membersTableArray count] == 0) {
            ...
            noResultLabel.text = @"عضوی برای پروژه تعریف نشده است";
            ...
            [membersTableView addSubview:noResultLabel];
        }
