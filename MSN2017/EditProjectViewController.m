...
@interface EditProjectViewController()<UITextFieldDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, UIPickerViewDataSource, UIPickerViewDelegate>
{
...
- (void)makeTopBar{
    ...
    titleLabel.text = @"ثبت پروفایل پروژه";
    if (_isFromEditProject) {
        titleLabel.text = @"ویرایش پروفایل پروژه";
    }
...
- (void)makePersonalInfo{
    ...
    nameTextField.placeholder = @"نام پروژه";
    ...
    //60x24
    UIImageView *takhasosImageView = [[UIImageView alloc]initWithFrame:CGRectMake(screenWidth - 40, horizontalLine3.frame.origin.y + 27, 60 / 3, 24 / 3)];
...
- (void)projMembersAction{
    ...
    view.membersArray = [[NSArray alloc]initWithArray:selectedTagsArray];
...
//    }
    ...
    NSDictionary *params = @{@"entity_type_id":[NSNumber numberWithInt:2],
...
- (void)editProfileToServer{
    ...
    if ([participantString length]  > 0) {
        ...
        [participantString insertString:@"[" atIndex:0];
        [participantString insertString:@"]" atIndex:1];
    }
...
- (void)getMembersFromServer{
    [ProgressHUD show:@""];
    NSDictionary *params = @{@"owner_id":[NSNumber numberWithInteger:[[_profileDictionary objectForKey:@"id"]integerValue]]
                             };
