...
@interface ThirdViewController ()<UITextFieldDelegate, UITextViewDelegate, UIImagePickerControllerDelegate,
...
- (void)makeBody{
    ...
    contentTextView.text = @"متن نوشته خود را تایپ کنید";
    ...
    selectCategoryButton = [CustomButton initButtonWithTitle:@"دسته بندی را انتخاب کنید" withTitleColor:[UIColor whiteColor] withBackColor:[UIColor colorWithRed:82/255.0 green:153/255.0 blue:223/255.0 alpha:1.0] isRounded:NO withFrame:CGRectMake(0, coverImageView.frame.origin.y + coverImageView.frame.size.height + 10, screenWidth, 35)];
    ...
    selectTagButton = [CustomButton initButtonWithTitle:@"تگ را انتخاب کنید" withTitleColor:[UIColor whiteColor] withBackColor:[UIColor colorWithRed:82/255.0 green:153/255.0 blue:223/255.0 alpha:1.0] isRounded:NO withFrame:CGRectMake(0, selectCategoryButton.frame.origin.y + selectCategoryButton.frame.size.height + 5, screenWidth, 35)];
    ...
    forLabel =[[UILabel alloc]initWithFrame:CGRectMake(screenWidth - 140, horizontalLine.frame.origin.y + 10, 130, 40)];
    forLabel.font = FONT_NORMAL(16);
    forLabel.text = @"نمایش پست در:";//NSLocalizedString(@"for:", @"");
    ...
    sendButton = [CustomButton initButtonWithTitle:@"ارسال" withTitleColor:[UIColor whiteColor] withBackColor:[UIColor colorWithRed:82/255.0 green:153/255.0 blue:223/255.0 alpha:1.0] isRounded:NO withFrame:CGRectMake(-5, screenHeight - 90, screenWidth + 5, 43)];
...
- (void)clearAllFields{
    ...
    contentTextView.text = @"متن نوشته خود را تایپ کنید";
    contentTextView.textColor = [UIColor grayColor];
    [selectCategoryButton setTitle:@"دسته بندی را انتخاب کنید" forState:UIControlStateNormal];
    selectedTagsArray = [[NSMutableArray alloc]init];
    selectCategoryID = 0;
    ...
    [namesScrollView addSubview:hintLable];
    [self deleteButtonAction];
    [self deleteButtonForAttachAction];
}
...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text {
    if ([textView.text isEqualToString:@"متن نوشته خود را تایپ کنید"]) {
        textView.text = @"";
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    if (isForAttachImage) {
        ...
        if ([[info objectForKey:@"UIImagePickerControllerMediaType"]containsString:@"movie"]) {
            NSString *urlString = [NSString stringWithFormat:@"%@", [info objectForKey:@"UIImagePickerControllerMediaURL"]];
            videoData = [NSData dataWithContentsOfURL:[NSURL URLWithString:urlString]];
            fileNameLabel.text = [NSString stringWithFormat:@"%lu.mp4", (unsigned long)[self getRandomNumberBetween:231354534 to:331354534]];//@"8393484142898394.jpg";
        } else {
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view{
    ...
    NSString *title;
    if (pickerView.tag == 1) {
        title = [[categoryArray myObjectAtIndex:row]objectForKey:@"name"];
    } else if (pickerView.tag == 2) {
        title = [[tagsArray myObjectAtIndex:row]objectForKey:@"name"];
    } else if (pickerView.tag == 3) {
        title = [[entityArray myObjectAtIndex:row]objectForKey:@"name"];
    }
...
-(void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component{
    if (pickerView.tag == 1) {
        [selectCategoryButton setTitle:[NSString stringWithFormat:@"%@", [[categoryArray myObjectAtIndex:row]objectForKey:@"name"]] forState:UIControlStateNormal];
        selectCategoryID = [[categoryArray myObjectAtIndex:row]objectForKey:@"id"];
    } else if (pickerView.tag == 2) {
        [hintLable removeFromSuperview];
        if ([selectedTagsArray count] > 0) {
            ///for (NSInteger i = 0; i < [selectedTagsArray count]; i++){
            if (![selectedTagsArray containsObject:[tagsArray myObjectAtIndex:row]]){
                if ([selectedTagsArray count] + 1 <= [tagsArray count]) {
                    [selectedTagsArray addObject:[tagsArray myObjectAtIndex:row]];
                    [self reloadTagsView];
                }
                //}
        ...
        [selectProfileButton setTitle:[NSString stringWithFormat:@"%@", [[entityArray myObjectAtIndex:row]objectForKey:@"name"]] forState:UIControlStateNormal];
        selectedEntityId = [[[entityArray myObjectAtIndex:row]objectForKey:@"id"]integerValue];
        ...
        NSString *name = [[entityArray myObjectAtIndex:row]objectForKey:@"name"];
        CGSize size = [self getSizeOfString: name];
        forWhomLabel = [[UILabel alloc]initWithFrame:CGRectMake(screenWidth - size.width - 140, forLabel.frame.origin.y + 10, size.width + 10, 25)];
...
/*
 ...
 tagLabel.text = [NSString stringWithFormat:@"%@", [[self.arrayOfProfessions myObjectAtIndex:i]objectForKey:@"name"]];
...
- (void)sendToServer{
    ...
    if ([selectCategoryButton.titleLabel.text isEqualToString:@"دسته بندی را انتخاب کنید"]) {
        UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"" message:@"دسته بندی را انتخاب کنید" preferredStyle:UIAlertControllerStyleAlert];
        UIAlertAction *defaultAction = [UIAlertAction actionWithTitle:@"بازگشت" style:UIAlertActionStyleDefault handler:^(UIAlertAction * action) {
            // ////NSLog(@"You pressed button OK");
        }];
        [alert addAction:defaultAction];
...
//        [params setValue:[[selectedTagsArray myObjectAtIndex:i]objectForKey:@"name"] forKey:keyname];
//    }
    
    NSMutableString *participantString = [[NSMutableString alloc]init];
    for (NSInteger i = 0; i < [selectedTagsArray count]; i++) {
        if (i < [selectedTagsArray count] - 1) {
            [participantString appendString:[NSString stringWithFormat:@"\"%@\",", [[selectedTagsArray myObjectAtIndex:i]objectForKey:@"name"]]];
        } else {
            [participantString appendString:[NSString stringWithFormat:@"\"%@\"]", [[selectedTagsArray myObjectAtIndex:i]objectForKey:@"name"]]];
        }
    }
    ...
    [manager POST:url parameters:params constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        ...
        if (videoData) {
            [formData appendPartWithFileData:videoData
                                        name:@"video"
                                    fileName:@"video" mimeType:@"video/mp4"];
        }
